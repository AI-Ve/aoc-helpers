...
def create_files(day: int, year: int) -> None:
    ...
    if not os.path.exists(year_dir):
        os.mkdir(year_dir)
        print(
            "Created new directory <-------- {} -------->".format("year " + str(year))
        )
    os.chdir(year_dir)
...
def get_puzzle(day: int, year: int) -> None:
    ...
    cookie = (
        ""  # Paste here the cookie you get when inspecting the AOC page while logged in
    )

    response = requests.get(
        input_url, cookies={"session": cookie}, allow_redirects=False
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        nargs=1,
        help="Years available: 2015-{}".format(
            current_time.year if current_time.month == 12 else current_time.year - 1
        ),
    )
    parser.add_argument("day", type=int, nargs=1, help="Days available: 1-25")
    args = parser.parse_args()

    day = args.day[0]
    year = args.year[0]

    if day not in range(1, 26) or year not in range(
        2015, current_time.year + 1 if current_time.month == 12 else current_time.year
    ):
